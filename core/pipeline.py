...
class VoicePipeline:
    ...
    def run(self):
        ...
        if self.input_mode == "voice":
            ...
            self._debug_print(
                f"{Colors.CYAN}[STT] ENDED | Total: {stt_metrics['total_time']:.3f}s | TTFT: {stt_metrics['ttft']:.3f}s{Colors.END}"  # noqa
            )
            ...
            if llm_chunk["type"] == "token":
                ...
                if self.output_mode == "voice":
                    ...
                    if trigger_punct or trigger_limit:
                        ...
                        self._debug_print(
                            f"\n{Colors.YELLOW}[PIPELINE] Chunking trigger: {reason} ({token_counter} tokens){Colors.END}"  # noqa
                        )
                ...
                self._debug_print(
                    f"\n\n{Colors.GREEN}[LLM] ENDED | Total: {llm_metrics['total_time']:.3f}s | TTFT: {llm_metrics['ttft']:.3f}s{Colors.END}"  # noqa
                )
                if self.output_mode == "voice":
                    self._debug_print(
                        f"{Colors.YELLOW}[TTS] FLOW SUMMARY | First TTFB: {tts_ttfb_initial:.3f}s | Total Synthesis: {tts_total_time:.3f}s{Colors.END}"  # noqa
                    )
        ...
        if self.debug:
            ...
            if stt_metrics:
                print(
                    f"STT: {stt_metrics['total_time']:.3f}s (TTFT: {stt_metrics['ttft']:.3f}s)"
                )
            if llm_metrics:
                print(
                    f"LLM: {llm_metrics['total_time']:.3f}s (TTFT: {llm_metrics['ttft']:.3f}s)"
                )
            if self.output_mode == "voice":
                print(
                    f"TTS: {tts_total_time:.3f}s (First TTFB: {tts_ttfb_initial:.3f}s)"
                )
